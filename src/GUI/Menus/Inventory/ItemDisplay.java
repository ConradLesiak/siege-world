...
public class ItemDisplay {
    ...
    public void create() {
        equipLabel = new Label("Equip", GUI.smallWhiteStyle);
        equipLabel.setPosition(Inventory.backgroundImage.getX() + Inventory.backgroundImage.getWidth() - equipLabel.getWidth(), 230);
        equipLabel.addListener( new ClickListener() {
            public void clicked(InputEvent event, float x, float y) {
                if(Items.getSelected() != null) {
                    ItemDisplay.removeStats();
                    PlayerStats.showStats();
                    Items.equipItem(Items.getSelected());
                    PlayerStats.updateStats();
                }
            }
        });
        dropLabel = new Label("Drop", GUI.smallWhiteStyle);
        dropLabel.setPosition(Inventory.backgroundImage.getX() + Inventory.backgroundImage.getWidth() - dropLabel.getWidth(), 70);
        dropLabel.addListener( new ClickListener() {
            public void clicked(InputEvent event, float x, float y) {
                if(Items.getSelected() != null) {
                    ItemDisplay.removeStats();
                    PlayerStats.showStats();
                    Items.dropItem(Items.getSelected());
                }
            }
        });
    ...
    public static void show() {
        ...
        Game.stageGUI.addActor(dropLabel);
    ...
    public static void showStats() {
        removeStats();
        if(Items.getSelected() != null) {
            PlayerStats.showDifference();
            //Name
            statsLabels.add(new Label(Items.getSelected().getName(), GUI.itemFontStyles.get(Items.getSelected().getLevel()-1)));
